...
public class JuegoNivel3V implements Screen {
	...
	public JuegoNivel3V(final KillBoss juego, int puntaje, int vidas) {
		this.juego = juego;
		...
		this.walkSheetDerecha = new Texture(Gdx.files.internal("sprites/valentinaSpriteBrazosD.png"));
		...
		this.walkSheetIzquierda = new Texture(Gdx.files.internal("sprites/valentinaSpriteBrazosI.png"));
		...
		this.valentinaNormal = new Texture(Gdx.files.internal("valentinaBrazos.png"));
	...
	public void render(float delta) {	
		...
		this.juego.texto.draw(this.juego.batch, "Nivel 3 - Cálculo Diferencial", 2048 / 2 - 500, 1010);
        ...
        if (this.numeroPreguntas > 0 && this.vidas > 0) {
           ...
           if (this.valentinaDerechaR.overlaps(BR) && this.azar % 2 != 0) {
        	...
        	if (this.numeroPreguntas == 0 && this.vidas > 0) {
        		this.juego.setScreen(new GanastePiernasV(this.juego, this.puntaje, this.vidas));
    			this.dispose();    			
        	}
        	else {
	...
	public String pregunta(int posicion) {	
		...
		this.db = a.abrir();		
		String pregunta = this.db.getPregunta(3, posicion);		
		return pregunta;
	}
	...
	public String respuesta(int posicion) {	
		Abrir a = new Abrir(this.juego);
		this.db = a.abrir();		
		String respuesta = this.db.getRespuesta(3, posicion);		
		return respuesta;
	}
	...
	public String posibleRespuesta(int posicion) {	
		Abrir a = new Abrir(this.juego);
		this.db = a.abrir();		
		String posibleRespuesta = this.db.getPosibleRespuesta(3, posicion);		
